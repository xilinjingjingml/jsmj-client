...
@ccclass
export default class jsmjDjdlDialog extends BaseUI {
    ...
    initButton() {
        izx.bindButtonClick("djdl/mflqBtn", this.node, () => {
            this.adOrderUpdateStatus()
            //this.pop()
        })
        izx.bindButtonClick("djdl/closeBtn", this.node, () => {
            this.pop()
        })
    }
    ...
    initParams (msg) {
        ...
        //     if (element.index == 0) {
        //         cc.find("djdl/moneyNum", this.node).getComponent(cc.Label).string = izx.getMoneyformat(Number(element.num)) + "元"
        //         return
        //     }
        // });
    }
